...
export function CriadorDashboard (data: { token: string }) {
    ...
    const schemaCobertura = z.object({
        "nomeCobertura": z.string().min(
            1,
            "Nome da cobertura não pode ficar vazio"
        ),
        "observacoes": z.string(),
        "tipoCobertura": z.string().min(
            1,
            "Selecione um tipo de cobertura"
        ),
        "fazendaCobertura": z.string().min(
            1,
    ...
    const schemaTransferencia = z.object({
        "adquirente": z.string().min(
            1,
            "Selecione adquirente"
        ),
        "animal": z.string().min(
            1,
            "Selecione um animal"
        ),
        "fazendaAdquirente": z.string().min(
            1,
            "Selecione uma fazenda"
        )
    });
    ...
    if (fazendas) {
        ...
        fazendaCobertura = fazendas.find((index: FazendaDTO) => index.id === coberturaSelecionada.fazendaCobertura);
    ...
    const handleCheckboxChangeMatriz = (animal: AnimalDTO) => {
        ...
        if (
            animaisSelecionadosMatriz.some((selectedAnimal) => selectedAnimal.id === animal.id)
        ) {

            setAnimaisSelecionadosCobertura((prev) => ({
                ...prev,
                "animaisSelecionadosMatriz": prev.animaisSelecionadosMatriz.filter((selectedAnimal: AnimalDTO) => selectedAnimal.id !== animal.id)
            }));
            ...
            setAnimaisSelecionadosCobertura((prev) => ({
                ...
                "animaisSelecionadosMatriz": [
                    ...prev.animaisSelecionadosMatriz,
                    animal
                ]
            }));
    ...
    const handleCheckboxChange = (animal: AnimalDTO) => {
        ...
        if (
            animaisSelecionados.some((selectedAnimal) => selectedAnimal.id === animal.id)
        ) {

            setAnimaisSelecionadosCobertura((prev) => ({
                ...prev,
                "animaisSelecionados": prev.animaisSelecionados.filter((selectedAnimal) => selectedAnimal.id !== animal.id)
            }));
    ...
    const submitCobertura = async (dataCobertura: ComunicacaoCoberturaDto) => {
        ...
        try {
            ...
            const response = await ComunicarCobertura(
                dataCoberturaApi,
                data.token
            );
            if (response.status == 201) {
                ...
                alert(
                    "Solicitação de cobertura cadastrada com sucesso",
                    "success"
                );
    ...
    async function handleSubmitNascimento (e) {
        ...
        const response = await criarComunicacaoNacimento(
            dataNascimento,
            data.token
        );
    ...
    async function solicitacaoAnimalBase (dataSolicitacao: SolicitacaoRegistroAnimalBaseDTO) {
        ...
        const response = await registrarAnimaisBase(
            dataAPI,
            data.token
        );
        ...
        if (!response.message) {
            ...
            const tecnicoUser = await getTecnicoEmail(
                data.token,
                dataSolicitacao.tecnicoId
            );
    ...
    async function getCoberturas () {
        ...
        if (coberturas.length == 0) {
            ...
            const response = await getAllCoberturas(
                data.token,
                criador.id
            );
        ...
        <Container>
                ...
                >
                    ...
                    >
                        ...
                        <Button
                            ...
                            colorButton={animalPage || verAnimalPage
                                ? "black"
                                : "#9E4B00"}
                            textButton="Animais"
                        />
                  ...
                  transferirPage
                        ...
                        >
                            ...
                            <Button
                                ...
                                marginLeftImage={"0.6vw"}
                                textSize="0.9vw"
                                textColor={comunicCoberturaPage
                                    ? "white"
                                    : "black"}
                                widthButton="100%"
                                widthImage="0.5vw"
                                ...
                                colorButton={comunicCoberturaPage
                                    ? "black"
                                    : "white"}
                            ...
                            <Button
                                ...
                                textSize="0.9vw"
                                textColor={
                                    comunicNascPage || verComunicNascPage
                                        ? "white"
                                        : "black"
                                }
                                widthButton="100%"
            ...
            <Content>
                    ...
                    >
                        ...
                        <Button
                            ...
                            onClick={() => {
                                ...
                                router.push(`/CadastrarRebanho/${data.token}`);
                        ...
                        <Button
                            ...
                            onClick={() => {
                                ...
                                router.push(`/CadastrarFazenda/${data.token}`);
                ...
                <ComunicCobertura
                    ...
                    style={{
                        "display": `${comunicCoberturaPage
                            ? "flex"
                            : "none"}`,
                        "pointerEvents": `${comunicCoberturaPage
                            ? "auto"
                            : "none"}`
                    }}
                ...
                Selecione uma fazenda
                        ...
                        {fieldNames.map((fieldName) => <div key={fieldName}>
                            <InputPlace>
                                <Text
                                    fontFamily="pop"
                                    size={"1.5vw"}
                                            ...
                                            if (data.nomeAnimal.toLocaleLowerCase().includes(numeroProcuradoF.toLocaleLowerCase()) && data.sexoAnimal == "Fêmea") {
                                                ...
                                                return (
                                                    <div style={{"display": "flex"}} key={data.id}>
                                                        ...
                                                        <Checkbox
                                                            key={data.id}
                                                            checked={animaisSelecionadosMatriz.some((selectedAnimal) => selectedAnimal.id === data.id)}
                                            ...
                                            if (data.nomeAnimal.toLocaleLowerCase().includes(numeroProcuradoM.toLocaleLowerCase()) && data.sexoAnimal == "Macho") {
                                                ...
                                                return (
                                                    <div style={{"display": "flex"}} key={data.id}>
                                                        ...
                                                        <Checkbox
                                                            key={data.id}
                                                            checked={animaisSelecionados.some((selectedAnimal) => selectedAnimal.id === data.id)}
                            ...
                            : paginate({"items": coberturas,
                                ...
                                <td>
                                    <Text
                                        ...
                                        fontFamily="rob"
                                        size={"1vw"}
                                        src={data.statusCobertura == "Em Análise"
                                            ? waiting
                                            : done}
                                        widthImage="1.5vw"
                ...
                Selecione uma fazenda adquirente
                            ...
                            {criadorAdquirente?.map((data: CriadorDTO) => {
                                ...
                                if (data.id === adquirente) {
                                    ...
                                    return data.fazenda.map((fazenda: FazendaDTO) => <option value={fazenda.id} key={fazenda.id}>
                                        {fazenda.nomeFazenda}
                                    </option>);
                ...
                <VerComunicNascimento
                    ...
                    style={{
                        "display": `${verComunicNascPage
                            ? "flex"
                            : "none"}`,
                        "pointerEvents": `${verComunicNascPage
                            ? "auto"
                            : "none"}`
                    }}
