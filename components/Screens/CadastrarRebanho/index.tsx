    ...
    const schema = z.object({
      ...
      criador?.rebanhos.length === 0
          ? z.
              ...
              min(
                  3,
                  "Rebanho deve ter no mínimo 3 caracteres"
              ).
              max(
                  3,
                  "Rebanho deve ter no máximo 3 caracteres"
              )
          : z.
              string().
              min(
                  1,
                  "Rebanho deve ter no mínimo 1 caracter"
              ).
              max(
                  1,
                  "Rebanho deve ter no máximo 1 caracter"
              ),
    ...
    async function cadastrarRebanho (data) {
        ...
        if (responseRebanho.serie) {
            ...
            if (criador.rebanhos.length === 0) {
                ...
                router.push(`/CadastrarRebanho/${props.token}`);

            }
            setLoading(false);
            return alert(
                "Rebanho cadastrado com sucesso!",
                "success"
            );
    ...
    return (
        <Container onSubmit={handleSubmit(cadastrarRebanho)}>
            ...
            >
                <Button
                    ...
                    onClick={() => {
                        ...
                        router.push(`/CriadorPage/${props.token}`);
          ...
          Selecione uma fazenda
                ...
                {fazendas.map((data: FazendaDTO) => <option value={data.id} key={data.nomeFazenda}>
                    {data.nomeFazenda}
                </option>)}
            </Select>
