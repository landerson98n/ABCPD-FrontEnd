...
export function AnimaisCriador (data: CriadoresABCPDDTO) {
    ...
    async function getFazendas () {
        ...
        const response = await getFazendaCriador(
            data.token,
            criadorId
        );
        if (response) {

            const rebanho = await getRebanhoByFazendaId(
                response[0].id,
                data.token
            );
    ...
    async function getInformacoesAnimal (animal: AnimalDTO) {
        ...
        onAnimalChange(
            "maeSelecionado",
            animaisCriador.find((index: AnimalDTO) => index.id == animal.mae) || {} as AnimalDTO
        );
        onAnimalChange(
            "paiSelecionado",
            animaisCriador.find((index: AnimalDTO) => index.id == animal.pai) || {} as AnimalDTO
        );
    ...
    return (
        <Content>
            ...
            <Table>
                ...
                {animaisCriador
                    ? animaisCriador.map((dataAnimal: AnimalDTO) => <TableContent key={dataAnimal.id}>
                        <td style={{"width": "20%"}}>
                            <Text
                                textAlign="center"
                                fontFamily="rob"
