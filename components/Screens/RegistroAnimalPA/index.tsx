...
export function RegistroAnimal (data: RegistroAnimalDTO) {
    ...
    async function updateSolicitacaoPa (solicitacao: SolicitacaoRegistroAnimalBaseDTO) {
        ...
        const response = await updateAnimaisPA(
            solData,
            data.token,
            solicitacao.id
        );
                    ...
                    ? solicitacoesAnimaisBase.map((index: SolicitacaoRegistroAnimalBaseDTO) => <TableContent key={index.criadorId}>
                        <td style={{"width": "20%"}}>
                            <Text
                                ...
                                size={"1vw"}
                                text={ todosRebanhos
                                    ? (todosRebanhos.find((reb:RebanhoDTO) => reb.id == index.rebanhoId) || {}).serie
                                    : ""}
                                color="black"
                                fontWeight="400"
                            />
                        ...
                        <td>
                            <Text
                                ...
                                size={"1vw"}
                                text={CriadoresData
                                    ? (CriadoresData?.find((cr: CriadorDTO) => cr.id == index.criadorId) || {}).nomeCompleto
                                    : null}
                                color="black"
                                fontWeight="400"
                            />
                        ...
                        <td style={{"width": "25%"}}>
                            <Text
                                widthImage="1.5vw"
                                src={index.estadoSolicitacao == "Em análise"
                                    ? waiting
                                    : done}
                                textAlign="center"
                                fontFamily="rob"
                                ...
                                <div style={{"marginLeft": "1vw"}}>
                                    <Button
                                        ...
                                        onClick={() => {
                                            ...
                                            onAnimalChange(
                                                "solicitacoesAnimaisBaseSelecionada",
                                                index
                                            );
                                            onCriadorChange(
                                                "criadorId",
