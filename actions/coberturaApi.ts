...
export async function ComunicarCobertura (data: { criadorCobertura: string; fazendaCobertura: string; nomeCobertura: string; observacoes: string; statusCobertura: string; tipoCobertura: string; finalizadoCobertura: boolean; pago: boolean; animais: AnimalDTO[]; }, token: string) {
    ...
    const res = await fetch(
        "https://abcpd-backend-production.up.railway.app/comunicacao-cobertura/cadastrar-cobertura",
        {
            "method": "POST",
...
export async function getAllCoberturas (token: string, id: string) {
    ...
    const res = await fetch(
        `https://abcpd-backend-production.up.railway.app/comunicacao-cobertura/get-coberturas-criador/${id}`,
        {
            "method": "GET",
            "headers": {
                "Authorization": `Bearer ${token}`
...
export async function getCoberturas (token: string) {
    ...
    const res = await fetch(
        "https://abcpd-backend-production.up.railway.app/comunicacao-cobertura/get-coberturas",
        {
            "method": "GET",
            "headers": {
                "Authorization": `Bearer ${token}`
...
export async function updateComunicCobertura (
    ...
    const res = await fetch(
        `https://abcpd-backend-production.up.railway.app/comunicacao-cobertura/update-cobertura/${id}`,
        {
            "method": "PUT",
            "body": JSON.stringify(data),
            "headers": {
